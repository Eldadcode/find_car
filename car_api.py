...
from config import (
    ISRAELI_API_BASE_URL,
    LICENSE_PLATE_RESOURCE_ID,
    LICENSE_PLATE_RESOURCE_ID_2,
    API_HEADERS,
)
...
class CarDataAPI:
    ...
    def _search_single_resource(
        self, license_plate: str, resource_id: str
    ...
    def search_car_by_license_plate(
        ...
        try:
            ...
            print(
                f"Not found in primary resource, trying secondary resource for license plate: {cleaned_plate}"
            )
    ...
    def has_handicapped_license(self, license_plate: str) -> bool:
        """Check if the car has a handicapped license (תו נכה)"""
        from config import HANDICAPPED_LICENSE_RESOURCE_ID

        try:
            params = {
                "resource_id": HANDICAPPED_LICENSE_RESOURCE_ID,
                "q": license_plate,
            }
            response = requests.get(
                self.base_url, params=params, headers=self.headers, timeout=10
    ...
    def format_car_info(self, car_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Add handicapped license info
            handicapped = self.has_handicapped_license(
                car_data.get("mispar_rechev", "")
            )
            if handicapped:
                car_info.append("**תו נכה:** כן")
            else:
                car_info.append("**תו נכה:** לא")
